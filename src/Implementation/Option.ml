...
module Tests = struct
  ...
  let test = "Option" >:: [
    ...
    "some_if' true" >: (lazy (check_some_int ~expected:42 (some_if' true 42)));
    ...
    "map Some" >: (lazy (check_some_int ~expected:42 (map ~f:(fun x -> 2 * x) (Some 21))));
    "value_map None" >: (lazy (check_int ~expected:57 (value_map ~def:57 ~f:(fun _ -> Exception.failure "Don't call me") None))); (*BISECT-IGNORE*)
    "value_map Some" >: (lazy (check_int ~expected:42 (value_map ~def:57 ~f:(fun x -> 2 * x) (Some 21))));
    "iter None" >: (lazy (iter ~f:(fun _ -> Exception.failure "Don't call me") None)); (*BISECT-IGNORE*)
    "iter Some" >: (lazy (check_int ~expected:42 (let x = ref 0 in iter ~f:(fun n -> x := n) (Some 42); !x)));
    "filter None" >: (lazy (check_none_int (filter ~f:(fun _ -> Exception.failure "Don't call me") None))); (*BISECT-IGNORE*)
    "filter Some true" >: (lazy (check_some_int ~expected:42 (filter ~f:(fun _ -> true) (Some 42))));
